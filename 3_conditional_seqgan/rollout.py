...
class ROLLOUT(object):
    ...
    def get_reward(self, sess, input_x, rollout_num, discriminator, type_idx):
        rewards = []
        for i in range(rollout_num):
            # given_num between 1 to sequence_length - 1 for a part completed sentence
            for given_num in range(1, self.sequence_length):
                feed = {self.x: input_x, self.given_num: given_num, self.type_index: type_idx}
                samples = sess.run(self.gen_x, feed)
                feed = {discriminator.input_x: samples, discriminator.input_cond: type_idx,
            ...
            feed = {discriminator.input_x: input_x, discriminator.input_cond: type_idx,
                   discriminator.dropout_keep_prob: 1.0}
            ypred_for_auc = sess.run(discriminator.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
